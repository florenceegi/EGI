...
export class UploadManager {
    ...
    private init(): void {
        ...
        this.dropzone.addEventListener('drop', (e) => this.handleDrop(e as DragEvent), false);
     ...
     */
    private async pollJobStatus(jobId: string, filename: string): Promise<void> {
        ...
        const poll = async (): Promise<void> => {
            try {
                ...
                if (result.status === 'completed') {
                    ...
                    if (result.act) {
                        window.dispatchEvent(new CustomEvent('natan:act-processed', {
                ...
                if (attempts < maxAttempts) {
                    ...
                    this.showError(`Timeout: processing troppo lungo per ${filename}`);
                }
    ...
    private setMessage(message: string, type: 'info' | 'success' | 'error'): void {
        ...
        let messageEl = this.dropzone.querySelector<HTMLDivElement>('.upload-message');
