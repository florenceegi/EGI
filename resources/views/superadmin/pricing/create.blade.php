<x-layouts.superadmin pageTitle="Aggiungi Feature Pricing">
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-base-content">➕ Aggiungi Feature Pricing</h1>
        <p class="mt-2 text-lg text-base-content/70">Crea una nuova feature pricing per la piattaforma</p>
    </div>

    <div class="card bg-base-100 shadow-xl">
        <div class="card-body">
            <form action="{{ route('superadmin.pricing.store') }}" method="POST">
                @csrf
                ...
                <div class="form-control mb-4">
                    ...
                    <input type="text" name="feature_code" value="{{ old('feature_code') }}"
                ...
                <div class="form-control mb-4">
                    ...
                    <textarea name="feature_description" rows="3"
                        ...
                        placeholder="Descrizione dettagliata della feature">{{ old('feature_description') }}</textarea>
                ...
                <div class="form-control mb-4">
                    ...
                    <select name="feature_category"
                        ...
                        <option value="ai_services" {{ old('feature_category') == 'ai_services' ? 'selected' : '' }}>
                            AI Services
                        </option>
                        <option value="premium_visibility"
                            {{ old('feature_category') == 'premium_visibility' ? 'selected' : '' }}>
                            Premium Visibility
                        </option>
                        <option value="premium_profile"
                            {{ old('feature_category') == 'premium_profile' ? 'selected' : '' }}>
                            Premium Profile
                        </option>
                        <option value="premium_analytics"
                            {{ old('feature_category') == 'premium_analytics' ? 'selected' : '' }}>
                            Premium Analytics
                        </option>
                        <option value="exclusive_access"
                            {{ old('feature_category') == 'exclusive_access' ? 'selected' : '' }}>
                            Exclusive Access
                        </option>
                        <option value="platform_services"
                            {{ old('feature_category') == 'platform_services' ? 'selected' : '' }}>
                            Platform Services
                        </option>
                    </select>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input type="number" name="cost_egili" value="{{ old('cost_egili') }}" step="1"
                            min="0" class="@error('cost_egili') input-error @enderror input input-bordered"
                            placeholder="es: 100">
                        @error('cost_egili')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                    ...
                    <div class="form-control mb-4">
                        ...
                        <input type="number" name="cost_fiat_eur" value="{{ old('cost_fiat_eur') }}" step="0.01"
                            min="0" class="@error('cost_fiat_eur') input-error @enderror input input-bordered"
                            placeholder="es: 9.99">
                        @error('cost_fiat_eur')
                            <label class="label">
                                <span class="label-text-alt text-error">{{ $message }}</span>
                ...
                <div class="form-control mb-4">
                    ...
                    <select name="min_tier_required"
                        ...
                        <option value="free" {{ old('min_tier_required') == 'free' ? 'selected' : '' }}>Free</option>
                        <option value="starter" {{ old('min_tier_required') == 'starter' ? 'selected' : '' }}>Starter
                        </option>
                        <option value="pro" {{ old('min_tier_required') == 'pro' ? 'selected' : '' }}>Pro</option>
                        <option value="business" {{ old('min_tier_required') == 'business' ? 'selected' : '' }}>
                            Business
                        </option>
                        <option value="enterprise" {{ old('min_tier_required') == 'enterprise' ? 'selected' : '' }}>
                            Enterprise
                        </option>
                    </select>
                ...
                <div class="form-control mb-6">
                    <label class="label cursor-pointer justify-start gap-4">
                        <input type="checkbox" name="is_active" value="1"
                            {{ old('is_active', true) ? 'checked' : '' }} class="checkbox-primary checkbox">
