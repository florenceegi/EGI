<x-layouts.superadmin :pageTitle="$pageTitle ?? 'Violazioni Padmin'">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-base-content/70 mt-1">{{ count($violations) }} violazioni trovate</p>
        </div>
    ...
    <div class="card bg-base-100 shadow-xl mb-6">
        <div class="card-body">
            <h3 class="card-title text-base mb-4">🔍 Filtri</h3>
            <form method="GET" action="{{ route('superadmin.padmin.violations') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                {{-- Priority Filter --}}
                <div class="form-control">
                    ...
                    <select name="priority" class="select select-bordered select-sm">
                        ...
                        <option value="P0" {{ request('priority') === 'P0' ? 'selected' : '' }}>P0 - Blocking</option>
                            ...
                            <tr class="hover:bg-base-200/50 transition-colors">
                                ...
                                <td>
                                    <span class="badge badge-sm font-mono
                                        {{ $violation['priority'] === 'P0' ? 'badge-error' : '' }}
                                        {{ $violation['priority'] === 'P1' ? 'badge-warning' : '' }}
                                        {{ $violation['priority'] === 'P2' ? 'badge-info' : '' }}
                                        {{ $violation['priority'] === 'P3' ? 'badge-neutral' : '' }}">
                                        {{ $violation['priority'] }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <span class="badge badge-sm badge-outline
                                        {{ $violation['severity'] === 'critical' ? 'badge-error' : '' }}
                                        {{ $violation['severity'] === 'error' ? 'badge-warning' : '' }}
                                        {{ $violation['severity'] === 'warning' ? 'badge-info' : '' }}">
                                ...
                                {{-- Type --}}
                                <td>
                                    <div class="font-semibold text-sm">{{ $violation['type'] }}</div>
                                    @if(isset($violation['message']))
                                        <div class="text-xs text-base-content/60 truncate max-w-xs" title="{{ $violation['message'] }}">
                                            {{ $violation['message'] }}
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td class="text-xs">
                                    @if(isset($violation['timestamp']))
                                        ...
                                        <div class="text-base-content/60">{{ date('H:i', $violation['timestamp']) }}</div>
                                    @else
                                ...
                                <td>
                                    @if(!($violation['isFixed'] ?? false))
                                        <button 
                                            onclick="markViolationFixed('{{ $violation['id'] }}')"
                                            class="btn btn-success btn-xs gap-1"
                                            title="Marca come risolta">
                                            <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                            </svg>
                                            Fix
    ...
    @else
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body text-center py-12">
                ...
                <p class="text-base-content/70 mt-2">Ottimo lavoro! Il codice rispetta tutte le regole OS3.0.</p>
