<x-platform-layout  :title="$user->name . ' - ' . __('creator.home.title_suffix')" :metaDescription="__('creator.home.meta_description', ['name' => $user->name])">
    ...
    <div class="pb-8 mb-8 space-y-8 border-solid border-slate-600">
                    ...
                    <div class="flex items-center justify-between mt-2">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                {{ auth()->user()->getCurrentBanner()->created_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            @else
            ...
            <form action="{{ route('profile.upload-banner') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="p-8 text-center transition-colors border-2 border-gray-300 border-dashed rounded-lg hover:border-oro-fiorentino hover:bg-gray-50/50"
                    ...
                    <label for="banner_image" class="block cursor-pointer">
                        ...
                        <p class="mt-2 text-sm text-gray-500">{{ __('profile.banner_help') }}
                        </p>
                    </label>
                </div>

                {{-- Preview Area (Hidden by default) --}}
                <div id="banner-preview-area" class="hidden space-y-4">
                    <h4 class="text-sm font-medium gdpr-subtitle">{{ __('profile.selected_images') }}</h4>
                    <div id="banner-preview-container" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <!-- Previews will be inserted here -->
                    </div>
                </div>

                {{-- Upload Button (Hidden by default) --}}
                <button type="submit" id="banner-upload-button"
        ...
        {{-- All Banner Images Card --}}
        @if (auth()->user()->getAllBannerImages()->count() > 0)
            <div class="p-6 gdpr-card rounded-2xl">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg gdpr-title">
                        {{ __('profile.all_banner_images') }}
                    </h3>
                    <span class="text-sm text-gray-500">{{ auth()->user()->getAllBannerImages()->count() }}
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                    @foreach (auth()->user()->getAllBannerImages() as $banner)
                        <div class="relative group">
                            <div class="w-full h-24 overflow-hidden rounded-lg">
                                <img src="{{ $banner->getUrl('banner_mobile') }}" alt="{{ __('profile.current_banner') }}"
                                    class="object-cover w-full h-full">
                            </div>

                            @if (auth()->user()->getCurrentBanner() && auth()->user()->getCurrentBanner()->getCustomProperty('source_media_id') == $banner->id)
                            ...
                            <div
                                ...
                                <div
                                    ...
                                    @if (!auth()->user()->getCurrentBanner() || auth()->user()->getCurrentBanner()->getCustomProperty('source_media_id') != $banner->id)
                                        <form action="{{ route('profile.set-current-banner') }}"
                                            ...
                                            <button type="submit"
                                                ...
                                                title="{{ __('profile.set_as_banner') }}">
                                    ...
                                    <form action="{{ route('profile.delete-banner') }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="media_id" value="{{ $banner->id }}">
                                        <button type="submit"
                                            ...
                                            title="{{ __('profile.delete_banner') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2"
                            ...
                            <div class="mt-2 text-xs text-gray-500 truncate">{{ $banner->name }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            // Banner Upload with Preview - Identical to profile images
            ...
            const bannerPreviewArea = document.getElementById('banner-preview-area');
            ...
            bannerFileInput.addEventListener('change', function(e) {
                const files = Array.from(e.target.files);
                selectedBannerFiles = files;

                if (files.length > 0) {
                    showBannerPreviews(files);
                    showBannerUploadButton();
                } else {
                    hideBannerPreviews();
                    hideBannerUploadButton();
                }
            });

            // Banner drag and drop functionality
            bannerUploadArea.addEventListener('dragover', function(e) {
                e.preventDefault();
                bannerUploadArea.classList.add('border-oro-fiorentino', 'bg-gray-50/50');
            });

            bannerUploadArea.addEventListener('dragleave', function(e) {
                e.preventDefault();
                bannerUploadArea.classList.remove('border-oro-fiorentino', 'bg-gray-50/50');
            });
            ...
            bannerUploadArea.addEventListener('drop', function(e) {
                ...
                const files = Array.from(e.dataTransfer.files).filter(file =>
                    file.type.startsWith('image/') && ['image/jpeg', 'image/png', 'image/webp', 'image/avif']
                    .includes(file.type)
                );
            ...
            function showBannerPreviews(files) {
                ...
                files.forEach((file, index) => {
                    ...
                    reader.onload = function(e) {
                        ...
                        previewDiv.innerHTML = `
                            ...
                            <div class="absolute inset-0 flex items-center justify-center transition-all duration-200 bg-black bg-opacity-0 rounded-lg group-hover:bg-opacity-50">
                                <button type="button" class="p-2 text-white transition-opacity duration-200 bg-red-500 rounded-full opacity-0 remove-banner-preview group-hover:opacity-100 hover:bg-red-600" data-index="${index}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                            ...
                            <div class="mt-2 text-xs text-gray-500 truncate">${file.name}</div>
                        `;
                        bannerPreviewContainer.appendChild(previewDiv);
                    };
                    reader.readAsDataURL(file);
                });

                bannerPreviewArea.classList.remove('hidden');
            }

            // Hide banner previews
            function hideBannerPreviews() {
                bannerPreviewArea.classList.add('hidden');
                bannerPreviewContainer.innerHTML = '';
            }

            // Show banner upload button
            function showBannerUploadButton() {
                bannerUploadButton.classList.remove('hidden');
            }

            // Hide banner upload button
            function hideBannerUploadButton() {
                bannerUploadButton.classList.add('hidden');
            }

            // Remove banner preview
            bannerPreviewContainer.addEventListener('click', function(e) {
                if (e.target.closest('.remove-banner-preview')) {
                    const index = parseInt(e.target.closest('.remove-banner-preview').dataset.index);
                    selectedBannerFiles.splice(index, 1);

                    if (selectedBannerFiles.length > 0) {
                        showBannerPreviews(selectedBannerFiles);
                    } else {
                        hideBannerPreviews();
                        hideBannerUploadButton();
            ...
            bannerUploadForm.addEventListener('submit', function(e) {
                ...
                fetch('{{ route('profile.upload-banner') }}', {
                    ...
                    .then(data => {
                        if (data.success) {
                            // Reload page to show new banners
                            window.location.reload();
                        } else {
                            alert(data.message || '{{ __('profile.banner_upload_failed') }}');
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        alert(error.message || '{{ __('profile.banner_upload_failed') }}');
                    })
                    .finally(() => {
                        // Reset button state
