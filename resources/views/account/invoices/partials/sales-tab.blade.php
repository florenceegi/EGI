...
<div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        {{-- Future: Add Create Invoice button --}}
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
                ...
                <select name="status" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                    ...
                    <option value="draft" {{ request('status') === 'draft' ? 'selected' : '' }}>{{ __('invoices.status.draft') }}</option>
                    <option value="pending" {{ request('status') === 'pending' ? 'selected' : '' }}>{{ __('invoices.status.pending') }}</option>
                    <option value="sent" {{ request('status') === 'sent' ? 'selected' : '' }}>{{ __('invoices.status.sent') }}</option>
                    ...
                    @foreach ($invoices as $invoice)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                {{ $invoice->buyer ? $invoice->buyer->name : __('common.multiple') }}
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
