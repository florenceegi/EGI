...
<div class="p-6 bg-white rounded-xl bg-opacity-10 backdrop-blur-md">
    ...
    @if($totalLikes > 0)
        <div class="space-y-3 max-h-60 overflow-y-auto">
            @foreach($usersWhoLiked as $userLike)
                @php
                    $user = $userLike['user'];
                    $avatarUrl = $user->profile_photo_url ?? 'https://dicebear.com/api/initials/' . urlencode($userLike['nickname']) . '.svg';
                    ...
                    if ($userObject) {
                        $profileRoute = match ($userObject->usertype ?? 'creator') {
                            ...
                            'commissioner' => route('profile.show', $userId), // Commissioner non ha pagina pubblica specifica
                            default => route('creator.home', $userId), // Fallback a creator
                        };
                    }
                @endphp
                
                <div class="flex items-center justify-between p-3 bg-black bg-opacity-20 rounded-lg hover:bg-opacity-30 transition-colors">
                    <div class="flex items-center space-x-3">
                        <a href="{{ $profileRoute }}" class="flex-shrink-0">
                            <img 
                                src="{{ $avatarUrl }}" 
                                alt="{{ $userLike['nickname'] }}"
                                class="w-10 h-10 rounded-full object-cover border-2 border-gray-600 hover:border-pink-400 transition-colors"
                                onerror="this.src='https://dicebear.com/api/initials/{{ urlencode($userLike['nickname']) }}.svg'"
                            >
                        </a>
                        <div>
                            <a href="{{ $profileRoute }}" class="font-medium text-white hover:text-pink-400 transition-colors">
                                {{ $userLike['nickname'] }}
                            </a>
    ...
    @else
        <div class="text-center py-8">
            ...
            <p class="text-gray-300 text-sm">{{ __('statistics.no_likes_received_yet') }}</p>
        </div>
    @endif
</div>