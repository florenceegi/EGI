                    ...
                    @foreach($givenLikesStats['liked_egis'] as $index => $egi)
                        ...
                        <div class="p-2 transition-colors bg-black rounded-lg bg-opacity-20 hover:bg-opacity-30">
                            <div class="flex items-center space-x-3">
                                {{-- Rank --}}
                                <div class="flex items-center justify-center flex-shrink-0 w-6 h-6 text-xs font-bold text-white rounded-full bg-gradient-to-r from-blue-500 to-cyan-500">
                                    {{ $index + 1 }}
                                </div>
                                ...
                                {{-- Like Badge --}}
                                <div class="flex-shrink-0">
                                    <div class="flex items-center justify-center w-6 h-6 text-xs font-bold text-white rounded-full bg-gradient-to-r from-blue-500 to-cyan-500">
                                        ❤️
                                    </div>
                                </div>
                                ...
                                <div class="flex-shrink-0 w-20">
                                    <div class="w-full h-2 bg-gray-700 rounded-full">
                                        <div class="h-2 rounded-full bg-gradient-to-r from-blue-500 to-cyan-500" style="width: {{ $percentage }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-2">
                    @foreach($givenLikesStats['owners'] as $index => $owner)
                        ...
                        <a href="{{ $ownerRoute }}" class="block p-2 transition-colors bg-black rounded-lg bg-opacity-20 hover:bg-opacity-30">
                            <div class="flex items-center space-x-3">
                                {{-- Rank --}}
                                <div class="flex items-center justify-center flex-shrink-0 w-6 h-6 text-xs font-bold text-white rounded-full bg-gradient-to-r from-purple-500 to-pink-500">
                                    {{ $index + 1 }}
                                </div>
                                ...
                                <div class="flex-shrink-0">
                                    @if($owner['user']->profile_photo_url)
                                        <img src="{{ $owner['user']->profile_photo_url }}" alt="{{ $owner['nick_name'] }}" class="object-cover w-8 h-8 rounded-full">
                                    @else
                                        <div class="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-purple-500 to-pink-500">
                                            <span class="text-xs font-bold text-white">{{ strtoupper(substr($owner['nick_name'], 0, 1)) }}</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                {{-- Owner Name --}}
                                <div class="flex-1 min-w-0">
                                    <div class="text-sm font-medium text-white truncate">{{ $owner['nick_name'] }}</div>
                                </div>
                                ...
                                <div class="flex-shrink-0 w-20">
                                    <div class="w-full h-2 bg-gray-700 rounded-full">
                                        <div class="h-2 rounded-full bg-gradient-to-r from-purple-500 to-pink-500" style="width: {{ $percentage }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </a>
                ...
                <div class="space-y-2">
                    @foreach($givenLikesStats['liked_egis'] as $index => $egi)
                        ...
                        <div class="flex items-center p-3 space-x-3 transition-all duration-200 bg-black rounded-lg bg-opacity-20 hover:bg-opacity-30">
                            {{-- Rank --}}
                            <div class="flex items-center justify-center flex-shrink-0 w-8 h-8 text-sm font-bold text-white rounded-full bg-gradient-to-r from-blue-500 to-purple-500">
                                {{ $index + 1 }}
                            </div>
                            ...
                            <div class="flex-shrink-0">
                                <a href="/egi/{{ $egi['id'] }}" class="block transition-transform hover:scale-105">
                                    @if($egi['avatar_image_url'])
                                        <img src="{{ $egi['avatar_image_url'] }}" alt="{{ $egi['title'] }}" class="object-cover w-12 h-12 border-2 border-blue-400 rounded-lg">
                                    @elseif($egi['thumbnail_image_url'])
                                        <img src="{{ $egi['thumbnail_image_url'] }}" alt="{{ $egi['title'] }}" class="object-cover w-12 h-12 border-2 border-blue-400 rounded-lg">
                                    @elseif($egi['main_image_url'])
                                        <img src="{{ $egi['main_image_url'] }}" alt="{{ $egi['title'] }}" class="object-cover w-12 h-12 border-2 border-blue-400 rounded-lg">
                                    @else
                                        <div class="flex items-center justify-center w-12 h-12 bg-gray-600 border-2 border-blue-400 rounded-lg">
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="/egi/{{ $egi['id'] }}" class="block transition-colors hover:text-blue-300">
                                    <div class="text-sm font-medium text-white truncate">{{ $egi['title'] ?? 'Untitled EGI' }}</div>
                                    <div class="text-xs text-gray-400 truncate">{{ $egi['collection_name'] ?? 'Unknown Collection' }}</div>
                                </a>
                                <a href="{{ $ownerRoute }}" class="block transition-colors hover:text-blue-300">
                                    <div class="text-xs text-blue-300">di {{ $egi['owner_name'] ?? 'Unknown Owner' }}</div>
                                </a>
                            </div>
                        </div>
                    @endforeach
            ...
            @if(count($givenLikesStats['owners']) > 0)
                <div class="space-y-2">
                    @foreach($givenLikesStats['owners'] as $index => $owner)
                        ...
                        <div class="flex items-center p-3 space-x-3 transition-all duration-200 bg-black rounded-lg bg-opacity-20 hover:bg-opacity-30">
                            {{-- Rank --}}
                            <div class="flex items-center justify-center flex-shrink-0 w-8 h-8 text-sm font-bold text-white rounded-full bg-gradient-to-r from-green-500 to-teal-500">
                                {{ $index + 1 }}
                            </div>
                            ...
                            <div class="relative flex-shrink-0">
                                ...
                                @else
                                    <div class="flex items-center justify-center w-10 h-10 border-2 border-green-400 rounded-full bg-gradient-to-r from-green-500 to-teal-500">
                                        <span class="text-sm font-bold text-white">{{ strtoupper(substr($owner['nickname'], 0, 1)) }}</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="{{ $ownerRoute }}" class="block transition-colors hover:text-green-300">
                                    ...
                                    <div class="text-xs text-gray-400">{{ $owner['likes_count'] }} like ricevuti da te</div>
                                </a>
                            </div>
                            ...
                            <div class="flex-shrink-0 w-20">
                                <div class="w-full h-2 bg-gray-700 rounded-full">
                                    <div class="h-2 rounded-full bg-gradient-to-r from-green-500 to-teal-500" style="width: {{ $percentage }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
