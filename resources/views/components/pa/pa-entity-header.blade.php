...
if ($showStats) {
    $stats = [
        'heritage_count' => $entity->collections()->whereType('pa_heritage')->withCount('egis')->get()->sum('egis_count') ?? 0,
...
<div
    {{ $attributes->merge(['class' => "pa-entity-header bg-gradient-to-r from-[#1B365D] to-[#0F2342] rounded-xl shadow-lg overflow-hidden border-2 border-[#D4A574]"]) }}
    role="banner"
    aria-label="Informazioni {{ $entityName }}"
>
    <div class="p-6 {{ $compact ? 'md:p-4' : 'md:p-8' }}">
        <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
            {{-- Logo/Avatar Section --}}
            <div class="flex-shrink-0">
                <div class="w-20 h-20 {{ $compact ? 'md:w-16 md:h-16' : 'md:w-24 md:h-24' }} rounded-full bg-white p-2 shadow-xl">
                    <img
                        src="{{ $logoUrl }}"
                        alt="Logo {{ $entityName }}"
            ...
            <div class="flex-grow">
                {{-- Entity Name --}}
                <h1 class="text-2xl {{ $compact ? 'md:text-xl' : 'md:text-3xl' }} font-bold text-white mb-2">
                    {{ $entityName }}
                </h1>

                {{-- Municipality --}}
                <div class="flex items-center gap-2 text-[#D4A574] mb-3">
                ...
                @if ($collection)
                    <div class="mb-3 p-3 bg-white/10 rounded-lg">
                        <p class="text-xs text-white/70 uppercase tracking-wide mb-1">Collezione</p>
                ...
                @if ($showContact && ($email || $phone || $website))
                    <div class="flex flex-wrap items-center gap-4 text-sm text-white/90">
                        @if ($email)
                            <a
                                href="mailto:{{ $email }}"
                                class="flex items-center gap-1 hover:text-[#D4A574] transition-colors"
            ...
            @if ($showStats && !empty($stats))
                <div class="flex-shrink-0 grid grid-cols-3 gap-4 {{ $compact ? 'md:gap-3' : 'md:gap-6' }} p-4 bg-white/10 rounded-lg">
                    {{-- Heritage Count --}}
                    <div class="text-center">
                        <p class="text-2xl font-bold text-[#D4A574]">{{ $stats['heritage_count'] }}</p>
                        <p class="text-xs text-white/70 uppercase tracking-wide mt-1">Beni</p>
                    </div>

                    {{-- CoA Issued --}}
                    <div class="text-center">
                        <p class="text-2xl font-bold text-[#D4A574]">{{ $stats['coa_issued'] }}</p>
                        <p class="text-xs text-white/70 uppercase tracking-wide mt-1">CoA</p>
                    </div>

                    {{-- Collections --}}
                    <div class="text-center">
                        <p class="text-2xl font-bold text-[#D4A574]">{{ $stats['collections'] }}</p>
                        <p class="text-xs text-white/70 uppercase tracking-wide mt-1">Collezioni</p>
                    </div>
                </div>
            @endif
        </div>
