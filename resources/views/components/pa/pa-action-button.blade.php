...
@php
    ...
    $variantClasses = [
        'primary' =>
            'bg-[#D4A574] hover:bg-[#C39463] text-white border-[#D4A574] hover:border-[#C39463] shadow-md hover:shadow-lg',
        'secondary' =>
            'bg-[#1B365D] hover:bg-[#0F2342] text-white border-[#1B365D] hover:border-[#0F2342] shadow-md hover:shadow-lg',
        'success' =>
            'bg-[#2D5016] hover:bg-[#1F3810] text-white border-[#2D5016] hover:border-[#1F3810] shadow-md hover:shadow-lg',
        'danger' =>
            'bg-[#C13120] hover:bg-[#A02718] text-white border-[#C13120] hover:border-[#A02718] shadow-md hover:shadow-lg',
        'outline' =>
            'bg-white hover:bg-gray-50 text-[#1B365D] border-[#1B365D] hover:border-[#D4A574] hover:text-[#D4A574]',
    ];
...
@if ($url && !$disabled)
    {{-- Link Button --}}
    <a href="{{ $url }}" target="{{ $target }}" {{ $attributes->merge(['class' => $baseClasses]) }}
        @if ($target === '_blank') rel="noopener noreferrer" @endif aria-label="{{ $label }}">
        @if ($icon)
...
@else
    ...
    <button type="{{ $type }}" {{ $attributes->merge(['class' => $baseClasses]) }}
        @if ($disabled) disabled aria-disabled="true" @endif aria-label="{{ $label }}">
        @if ($icon)
            <span class="material-symbols-outlined {{ $iconSize }}" aria-hidden="true">{{ $icon }}</span>
        @endif
        <span>{{ $label }}</span>
