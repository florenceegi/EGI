...
{{-- CoA Main Section --}}
<div class="bg-gradient-to-br from-amber-900/20 to-yellow-900/20 rounded-2xl border border-amber-500/30 backdrop-blur-sm p-6 space-y-4">
    ...
    <div class="flex items-center space-x-3">
        <div class="flex-shrink-0">
            ...
            @else
                ...
                <div class="w-10 h-10 bg-gradient-to-br from-gray-600 to-gray-700 rounded-xl flex items-center justify-center shadow-lg">
    ...
    @if($hasActiveCoa)
        <div class="space-y-3 border-t border-amber-500/20 pt-4">
            {{-- Certificate Details --}}
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-300">{{ __('egi.coa.serial_number') }}:</span>
                    <span class="text-white font-mono text-xs">{{ $existingCoa->serial }}</span>
                </div>
                <div class="flex justify-between text-sm">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="text-amber-400 font-medium">{{ $existingCoa->annexes_count }} {{ __('egi.coa.pro') }}</span>
                </div>
                @endif
            </div>
            ...
            {{-- Quick Actions for Active Certificate --}}
                ...
                <a href="{{ route('coa.verify.view', $existingCoa->verification_hash) }}"
                   target="_blank"
                   class="flex-1 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-amber-900 font-medium py-2 px-3 rounded-lg text-sm text-center transition-all duration-200 shadow-lg hover:shadow-xl">
                ...
                <button onclick="downloadCoaBundle({{ $existingCoa->id }})"
                        class="flex-1 bg-gray-700 hover:bg-gray-600 text-white font-medium py-2 px-3 rounded-lg text-sm transition-all duration-200">
                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($canManageCoa)
                <div class="border-t border-amber-500/20 pt-3">
                    <details class="group">
                        <summary class="flex items-center justify-between cursor-pointer text-sm text-gray-300 hover:text-white transition-colors">
                        ...
                        <div class="mt-3 space-y-2">
                            ...
                            <button onclick="openCoaAnnexModal({{ $existingCoa->id }})"
                                    class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-3 rounded-lg text-sm transition-all duration-200">
                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="reissueCoaCertificate({{ $existingCoa->id }})"
                                    class="w-full bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-3 rounded-lg text-sm transition-all duration-200">
                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="revokeCoaCertificate({{ $existingCoa->id }})"
                                    class="w-full bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-3 rounded-lg text-sm transition-all duration-200">
                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-3 border-t border-amber-500/20 pt-4">
            ...
            @if($canIssueCoa)
                ...
                <button onclick="issueCoaCertificate({{ $egi->id }})"
                        class="w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-bold py-3 px-4 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl">
                ...
                <div class="bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-lg p-3 border border-blue-500/20">
                    <h4 class="text-sm font-medium text-blue-300 mb-2">{{ __('egi.coa.unlock_pro') }}:</h4>
