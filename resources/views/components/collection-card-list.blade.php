...
<article
    class="collection-card-list group relative rounded-xl border border-gray-700/50 bg-gray-800/50 p-4 transition-all duration-300 hover:border-gray-600 hover:bg-gray-800/70"
    data-collection-id="{{ $collection->id }}">

    <div class="flex items-start gap-4">
        <!-- Image Section -->
        <a href="{{ route('home.collections.show', $collection->id) }}"
            class="relative h-28 w-28 flex-shrink-0 cursor-pointer overflow-hidden rounded-lg bg-gradient-to-br from-gray-700 to-gray-800 transition-all duration-300 group-hover:ring-2 group-hover:ring-blue-400">
            ...
            @else
                <div
                    class="flex h-full w-full items-center justify-center bg-gradient-to-br from-blue-600/20 to-purple-600/20">
            ...
            <div
                class="absolute inset-0 flex items-center justify-center bg-blue-400/20 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            ...
            @if ($showBadge)
                <div class="absolute -right-1 -top-1">
                    <div class="{{ $config['badge_color'] }} flex h-6 w-6 items-center justify-center rounded-full ring-2 ring-gray-800"
        ...
        <div class="mr-4 min-w-0 flex-1">
            ...
            <div class="mb-1 flex items-start justify-between">
                <h3 class="flex-1 truncate text-lg font-bold text-white transition-colors group-hover:text-blue-300">
            ...
            @if ($config['show_creator'] && $collection->user)
                <div class="mb-2 flex items-center gap-2 text-sm text-gray-400">
                    <div class="h-3 w-3 rounded-full bg-gradient-to-r from-blue-500 to-purple-500"></div>
                    <span class="max-w-[120px] truncate">{{ $collection->user->first_name }}
                        {{ $collection->user->last_name }}</span>
            ...
            <div class="mb-2 flex flex-wrap items-center gap-4 text-sm text-gray-400">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <span
                        class="font-medium text-blue-300">{{ $collection->original_egis_count ?? ($collection->egis_count ?? 0) }}</span>
                    <span>EGI</span>
                </div>
            ...
            @if (isset($collection->search_user_roles) && count($collection->search_user_roles))
                <div class="mt-2 flex flex-wrap gap-1">
                    @foreach ($collection->search_user_roles as $userName => $roles)
                        <div class="rounded bg-gray-700/60 px-2 py-1 text-xs text-gray-300">
            ...
            <div class="mt-2 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <span
                        class="inline-flex items-center rounded-full bg-gradient-to-r from-blue-500 to-purple-500 px-2 py-1 text-xs font-medium text-white">
                        {{ __('label.collection') }}
                    </span>
                </div>
