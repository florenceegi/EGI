...
<x-pa-layout title="Modifica Bene Culturale">
    <x-slot:breadcrumb>
        ...
        <span class="mx-2 text-gray-400">/</span>
        <a href="{{ route('egis.show', $egi) }}" class="text-[#D4A574] hover:text-[#C39463]">{{ Str::limit($egi->title, 30) }}</a>
        <span class="mx-2 text-gray-400">/</span>
    ...
    <div class="mx-auto mb-6 max-w-4xl rounded-lg bg-amber-50 border-l-4 border-amber-500 p-4">
        <div class="flex items-start">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-amber-700">
                    ...
                    Le modifiche ai dati principali richiederanno una nuova verifica da parte dell'ente certificatore.
                </p>
            </div>
        </div>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="collection_id" 
                        ...
                        @foreach($collections as $collection)
                            <option value="{{ $collection->id }}" {{ $egi->collection_id == $collection->id ? 'selected' : '' }}>
                                {{ $collection->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           required
                           maxlength="255"
                           placeholder="Es: Statua di David, Palazzo Vecchio, etc."
                ...
                <div>
                    ...
                    <input type="text" 
                           id="artist" 
                           name="artist" 
                           value="{{ old('artist', $egi->artist) }}"
                           required
                           maxlength="255"
                           placeholder="Es: Michelangelo, Leonardo da Vinci, etc."
                           class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-[#1B365D] focus:outline-none focus:ring-2 focus:ring-[#1B365D]/20">
                    @error('artist')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" 
                           id="creation_date" 
                           name="creation_date" 
                           value="{{ old('creation_date', $egi->creation_date) }}"
                           required
                           placeholder="Es: 1504, XVI secolo, 1450-1460, etc."
                           maxlength="100"
                           class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-[#1B365D] focus:outline-none focus:ring-2 focus:ring-[#1B365D]/20">
                    @error('creation_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <label for="image" 
                            ...
                            <p class="mb-1 text-sm font-medium text-[#1B365D]" x-show="fileName" x-text="fileName"></p>
                ...
                {{-- CoA Info (if exists) --}}
                @if($egi->coa)
                <div class="rounded-lg bg-[#1B365D]/5 p-4 border border-[#1B365D]/20">
                    ...
                    <div class="grid gap-3 text-sm md:grid-cols-2">
                        <div>
                            ...
                            <span class="ml-2 rounded px-2 py-1 text-xs font-medium
                                @if($egi->coa->status === 'verified') bg-green-100 text-green-800
                        ...
                        <div>
                            ...
                            <span class="ml-2 text-gray-600">{{ $egi->coa->issued_at->format('d/m/Y') }}</span>
                        </div>
                        @endif
                    </div>
