...
<x-guest-layout :title="$collector->name . ' - ' . __('collector.home.title_suffix')" :metaDescription="__('collector.home.meta_description', ['name' => $collector->name])">
    ...
    <x-slot name="platformInfoButtons">
        ...
        @if ($isOwnerViewing)
            <div class="absolute z-30 hidden bottom-4 right-4 md:block">
                <button onclick="window.paymentModal.open()" aria-label="{{ __('payment.settings_title') }}"
            ...
            <div id="payment-fab-mobile" class="fixed z-50 bottom-20 right-4 md:hidden">
                <button onclick="window.paymentModal.open()" aria-label="{{ __('payment.settings_title') }}"
                ...
                @if ($isOwnerViewing)
                    <div class="absolute z-10 right-4 top-4">
                        <button onclick="openImageModal('collector-banner-modal')"
                            class="px-4 py-2 text-sm font-medium text-white transition-all rounded-lg hover:bg-oro-fiorentino bg-gray-900/80 backdrop-blur-sm hover:text-gray-900">
            ...
            <div class="p-4 border rounded-2xl border-white/10 bg-white/5 backdrop-blur-xl">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative flex-shrink-0 group">
                        <div class="w-20 h-20 overflow-hidden rounded-full ring-oro-fiorentino/40 ring-2">
                    ...
                    <div class="flex-1 min-w-0">
                        <h1 class="text-xl font-bold text-white truncate">{{ $collector->name }}</h1>
                        @if ($collector->tagline)
                            <p class="text-sm italic truncate text-oro-fiorentino/80">"{{ $collector->tagline }}"</p>
                        @endif
                ...
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <div class="rounded-xl bg-white/5 px-2 py-2.5 text-center">
                        <p class="text-lg font-bold text-oro-fiorentino">{{ $stats['collections_represented'] ?? 0 }}
                        </p>
                        <p class="text-[9px] uppercase tracking-wider text-gray-400">
                            {{ __('collector.collections_represented') }}</p>
                    </div>
                    <div class="rounded-xl bg-white/5 px-2 py-2.5 text-center">
                        <p class="text-base font-bold text-oro-fiorentino">
        ...
        <div class="relative z-10 hidden px-4 py-16 mx-auto max-w-7xl sm:px-6 md:block md:py-24 lg:px-8">
            <div class="grid items-end grid-cols-1 gap-12 md:grid-cols-12 md:gap-8">
                <div class="flex flex-col items-center gap-6 sm:flex-row sm:items-end sm:gap-8 md:col-span-8">
                    <div class="relative flex-shrink-0 group">
                        <div
                            class="w-32 h-32 overflow-hidden rounded-full shadow-2xl ring-oro-fiorentino/40 ring-4 md:h-40 md:w-40">
                            <img src="{{ $collector->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($collector->name) . '&size=160&background=D4A574&color=2D5016' }}"
                                alt="{{ __('collector.home.avatar_alt', ['name' => $collector->name]) }}"
                        ...
                        @if ($isOwnerViewing)
                            <button onclick="openImageModal('collector-avatar-modal')"
                                class="absolute bottom-0 right-0 p-2 text-gray-900 transition-opacity rounded-full shadow-lg opacity-0 bg-oro-fiorentino hover:bg-oro-fiorentino/90 group-hover:opacity-100">
                    ...
                    <div class="flex flex-col text-center sm:text-left">
                        ...
                        @if ($collector->tagline)
                            <p class="text-lg italic font-source-sans text-oro-fiorentino md:text-xl">
                                "{{ $collector->tagline }}"
                            </p>
                        @endif
    ...
    <x-slot name="platformStats">
        ...
        <nav class="flex overflow-x-auto border-b border-gray-800 bg-gray-900/95"
            ...
            <div class="flex w-full px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex w-full space-x-6 scrollbar-hide">
    ...
    <x-slot name="heroFullWidth">
        <section class="bg-gray-900">
            <div class="px-4 py-12 mx-auto space-y-12 max-w-7xl sm:px-6 lg:px-8">
                {{-- Portfolio Preview --}}
                ...
                @else
                    <div
                        class="px-6 py-12 text-center border border-gray-700 border-dashed rounded-2xl bg-gray-900/60">
                        ...
                        <a href="{{ route('home.collections.index') }}"
                            class="inline-flex items-center px-6 py-3 font-semibold text-white transition-colors duration-200 bg-purple-600 rounded-full hover:bg-purple-700">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            @foreach ($collectorCollections->take(6) as $collection)
                                <article
                                    class="overflow-hidden transition-all duration-300 border border-gray-800 hover:border-oro-fiorentino/60 rounded-xl bg-gray-900/70 hover:shadow-2xl">
                                    <div class="p-6">
                                        ...
                                        <div class="flex items-center justify-between text-sm text-gray-300">
                                            ...
                                            @if ($collection->total_value)
                                                <span
                                                    class="font-medium text-oro-fiorentino">€{{ number_format($collection->total_value, 2) }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </article>
