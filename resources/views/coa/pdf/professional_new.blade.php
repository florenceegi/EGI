...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .verified-banner {
            background-color: #2D5016; /* VERDE RINASCITA */
            color: #fff;
            padding: 8pt 12pt;
            margin-bottom: 15pt;
            text-align: center;
        ...
        .signature-line { border-top: 1px solid #333; margin: 40pt 20pt 0 20pt; padding-top: 5pt; font-size: 9pt; color: #6B6B6B; }
        .stamp-area { height: 80pt; border: 2px dashed #cccccc; margin: 0 20pt; line-height: 80pt; color: #aaaaaa; }
        .stamp-caption { font-size: 9pt; color: #6B6B6B; margin-top: 5pt; }
        ...
        .technical-table {
            ...
            text-align: center;
            font-size: 8.5pt; /* Riduciamo leggermente il font per compattare */
        }
...
<body>
    ...
    <div class="section">
        ...
        <table class="info-table">
            <tr><td class="info-label">{{ __('coa_traits.pdf_title') }}:</td><td class="info-value">{{ $egi->title ?? __('coa_traits.no_title_available') }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_internal_id') }}:</td><td class="info-value">#{{ $egi->internal_id ?? $egi->id }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_author') }}:</td><td class="info-value">{{ $creator->name ?? __('coa_traits.no_artist_available') }}</td></tr>
        </table>
    </div>

    @php
        $grouped_traits = isset($traits_snapshot) && is_array($traits_snapshot) ? collect($traits_snapshot)->groupBy('category') : collect();
    @endphp
    ...
    <div class="section">
        ...
        <table class="info-table">
            <tr><td class="info-label">{{ __('coa_traits.pdf_certificate_id') }}:</td><td class="info-value">{{ $serial }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_creation_date') }}:</td><td class="info-value">{{ $egi->created_at_formatted ?? $egi->created_at->format('d/m/Y H:i') }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_hash_title') }}:</td><td class="info-value hash-value">{{ $file_hash ?? hash('sha256', $serial) }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_issued_by') }}:</td><td class="info-value">{{ $egi->issued_by ?? __('coa_traits.pdf_powered_by') }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_issue_place') }}:</td><td class="info-value">{{ $egi->issue_place ?? __('coa_traits.pdf_florence_italy') }}</td></tr>
            <tr><td class="info-label">{{ __('coa_traits.pdf_status') }}:</td><td class="info-value">{{ __('coa_traits.pdf_status_valid') }}</td></tr>
        </table>
    </div>
    ...
    <div class="verification">
        ...
        <div style="display: table; width: 100%;">
            <div style="display: table-cell; vertical-align: middle; padding-right: 15px;">{{ __('coa_traits.pdf_scan_prompt') }}</div>
            @if (isset($qr_code))
    ...
    @php
        $grouped_metadata = isset($additional_metadata) && is_array($additional_metadata) ? collect($additional_metadata)->groupBy('category') : collect();
    @endphp
