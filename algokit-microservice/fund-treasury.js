...
const treasuryAddress = 'Y2IGWQ5ZL2LBSNBQCKFC3QDRJFGXSJGYB5IWSXPDTHTF6UTBJTMEU5LPYE';

async function fundTreasury() {
    try {
        console.log('🏦 Funding Treasury Wallet from Sandbox Default Account...\n');
        ...
        const dispenserInfo = await algodClient.accountInformation(dispenserAddress).do();
        ...
        const txResponse = await algodClient.sendRawTransaction(signedTxn.sig).do();
        ...
        const treasuryInfo = await algodClient.accountInformation(treasuryAddress).do();
        console.log(`🎉 Treasury Balance: ${treasuryInfo.amount / 1000000} ALGO`);
