...
(function() {
    ...
    function initializeElements() {
        ...
        state.messagesContainer = document.getElementById('art-advisor-messages');
    ...
    function attachEventListeners() {
        ...
        expertBtns.forEach(btn => {
            btn.addEventListener('click', () => selectExpert(btn.dataset.expert));
        });
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape' && !state.modal.classList.contains('hidden')) {
                closeModal();
            }
        });
    }
    ...
    function selectExpert(expertId) {
        ...
        expertBtns.forEach(btn => {
            if (btn.dataset.expert === expertId) {
                btn.classList.add('active', 'bg-gradient-to-r', 'from-purple-600', 'to-blue-600', 'text-white', 'shadow-lg');
                btn.classList.remove('bg-gray-700', 'text-gray-300');
            } else {
                btn.classList.remove('active', 'bg-gradient-to-r', 'from-purple-600', 'to-blue-600', 'text-white', 'shadow-lg');
    ...
    function toggleVision() {
        ...
        if (state.visionEnabled) {
            visionToggle.classList.add('bg-blue-600', 'border-blue-500', 'text-white');
            visionToggle.classList.remove('bg-gray-700/50', 'border-gray-600', 'text-gray-300');
            if (visionText) {
                visionText.textContent = '🔍 Vision Attiva';
            }
        } else {
            visionToggle.classList.remove('bg-blue-600', 'border-blue-500', 'text-white');
            visionToggle.classList.add('bg-gray-700/50', 'border-gray-600', 'text-gray-300');
            if (visionText) {
    ...
    function showWelcomeMessage() {
        ...
        const message = welcomeMessages[state.mode] || welcomeMessages['general'];
    ...
    function addMessage(role, content, expert = null) {
        ...
        if (role === 'ai' && expert) {
            ...
            expertBadge.textContent = expert === 'creative' ? '🎨 Creative Advisor' : '📖 Platform Assistant';
    ...
    function createActionButtons(content) {
        ...
        copyBtn.className = 'flex items-center gap-1 rounded-lg bg-gray-600/50 px-3 py-1.5 text-xs font-medium text-gray-200 transition-colors hover:bg-gray-600';
        ...
        applyBtn.className = 'flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-green-700';
    ...
    function sendChatRequest(message) {
        ...
        .catch(error => {
            ...
            updateStreamingMessage(aiMessageDiv, 'Si è verificato un errore. Riprova tra poco.', true);
        })
        .finally(() => {
            ...
            lines.forEach(line => {
                if (line.startsWith('event:')) {
                    ...
                    try {
                        ...
                        console.error('[ArtAdvisor] Failed to parse SSE data', e);
                    }
                }
            });
    ...
    function createStreamingMessage() {
        ...
        const bubble = document.createElement('div');
        bubble.className = 'max-w-[80%] rounded-2xl border border-gray-600/50 bg-gray-700/50 px-4 py-3';

        // Expert badge
        const badge = document.createElement('div');
        badge.className = 'mb-2 text-xs font-medium text-blue-400';
        badge.textContent = state.currentExpert === 'creative' ? '🎨 Creative Advisor' : '📖 Platform Assistant';
        bubble.appendChild(badge);

        // Text content
        const textDiv = document.createElement('div');
        textDiv.className = 'whitespace-pre-wrap text-sm leading-relaxed text-gray-100';
        textDiv.textContent = '...';
        bubble.appendChild(textDiv);

        // Streaming indicator
        const indicator = document.createElement('div');
        indicator.className = 'streaming-indicator mt-2 flex items-center gap-1 text-xs text-gray-400';
        indicator.innerHTML = '<span class="animate-pulse">●</span><span>Streaming...</span>';
