...
class EgiPreMintManagementService {
    ...
    public function generateDescription(Egi $egi, User $user, array $requestMetadata, ?string $creatorGuidelines = null): array {
        try {
            ...
            $this->logger->info('[PRE_MINT_SERVICE] Description generated by AI, saving to DB', [
                'egi_id' => $egi->id,
                'description_length' => strlen($generatedDescription),
                'description_preview' => substr($generatedDescription, 0, 100),
                'log_category' => 'PRE_MINT_GENERATE_DESCRIPTION_BEFORE_SAVE'
            ]);

            // 6. Save description to database
            ...
            // 7. Verify save and log
            $egi->refresh(); // Reload from DB
            $this->logger->info('[PRE_MINT_SERVICE] Description saved to DB, verifying', [
                'egi_id' => $egi->id,
                'db_description_length' => strlen($egi->description ?? ''),
                'db_description_preview' => substr($egi->description ?? '', 0, 100),
                'save_successful' => !empty($egi->description),
                'log_category' => 'PRE_MINT_GENERATE_DESCRIPTION_AFTER_SAVE'
            ]);

            // 8. GDPR: Log user action with AuditLogService
    ...
    public function improveDescription(Egi $egi, User $user, array $requestMetadata): array {
        try {
            ...
            // 6. Log description before save
            $this->logger->info('[PRE_MINT_SERVICE] Description improved by AI, saving to DB', [
                'egi_id' => $egi->id,
                'description_length' => strlen($improvedDescription),
                'description_preview' => substr($improvedDescription, 0, 100),
                'log_category' => 'PRE_MINT_IMPROVE_DESCRIPTION_BEFORE_SAVE'
            ]);

            // 7. Save improved description to database
            ...
            // 8. Verify save and log
            $egi->refresh(); // Reload from DB
            $this->logger->info('[PRE_MINT_SERVICE] Improved description saved to DB, verifying', [
                'egi_id' => $egi->id,
                'db_description_length' => strlen($egi->description ?? ''),
                'db_description_preview' => substr($egi->description ?? '', 0, 100),
                'save_successful' => !empty($egi->description),
                'log_category' => 'PRE_MINT_IMPROVE_DESCRIPTION_AFTER_SAVE'
            ]);

            // 9. GDPR: Log user action with AuditLogService
