...
class CoaEvent extends Model
{
    ...
    public const TYPE_SIGNATURE_REMOVED = 'SIGNATURE_REMOVED';
    ...
    /**
     * Create event for signature removal
     */
    public static function createSignatureRemoved(
        Coa $coa,
        User $actor,
        string $role,
        array $signatureInfo,
        ...
        return static::create([
            'coa_id' => $coa->id,
            'type' => self::TYPE_SIGNATURE_REMOVED,
            'payload' => [
                'event' => ucfirst($role) . ' signature removed',
                'role' => $role,
                'signature_info' => $signatureInfo,
                'reason' => $reason,
                'timestamp' => now()->toISOString(),
    ...
    public function getDisplayDescription(): string
    {
        ...
        return match ($this->type) {
            ...
            self::TYPE_SIGNATURE_REMOVED => ucfirst($this->payload['role'] ?? 'Unknown') . " signature removed by {$actorName}",
    ...
    public function getColorClass(): string
    {
        return match ($this->type) {
            ...
            self::TYPE_SIGNATURE_REMOVED => 'text-red-600',
