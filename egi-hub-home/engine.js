...
// HELPER: Create Text Texture (Canvas)
function createTextTexture(text, subtext) {
    ...
    // Title
    // Fill ENTIRE CANVAS with solid black for MAXIMUM contrast
    ctx.fillStyle = '#000000'; // SOLID BLACK
    ctx.fillRect(0, 0, canvas.width, canvas.height); // ENTIRE CANVAS BLACK
    
    // Font scaled UP to be clearly visible
    ctx.font = 'bold 180px "Rajdhani"'; 
    ctx.fillStyle = '#ffffff';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.shadowColor = "rgba(0, 0, 0, 1.0)"; // Full black shadow
    ctx.shadowBlur = 8;
    ctx.shadowOffsetX = 3;
    ctx.shadowOffsetY = 3;
    
    // Center of the canvas (Front of the sphere)
    ctx.fillText(text.toUpperCase(), canvas.width / 2, canvas.height / 2);
    
    // Subtext
    if(subtext) {
        ctx.font = '100px "Share Tech Mono"'; // Larger subtext
...
function createComplexNode(id, data, radius) {
    ...
    // 2. INTERNAL LABEL (Spherical Text - INSIDE Glass)
    // Now visible because glass transmission is disabled.
    const textTex = createTextTexture(data.label, data.tagline);
    const labelGeo = new THREE.SphereGeometry(radius * 0.85, 32, 16); // INSIDE glass (0.85 < 1.0)
    const labelMat = new THREE.MeshBasicMaterial({ 
        map: textTex, 
        transparent: false, // SOLID: Black background not see-through
        side: THREE.DoubleSide, // Visible from all angles
        depthWrite: true // Write to depth buffer
    });
    const labelMesh = new THREE.Mesh(labelGeo, labelMat);
    labelMesh.renderOrder = 1; // After core, before glass
    root.add(labelMesh);
...
function animate() {
    ...
    Object.values(nodes).forEach(n => {
        ...
        // Billboard Labels: Face camera using world-space quaternion
        if(n.labelMesh) {
            // Get parent's world quaternion and invert it
            const parentWorldQuat = new THREE.Quaternion();
            n.root.getWorldQuaternion(parentWorldQuat);
            parentWorldQuat.invert();
            
            // Combine with camera quaternion to get local rotation that faces camera
            const targetQuat = parentWorldQuat.multiply(camera.quaternion);
            n.labelMesh.quaternion.copy(targetQuat);
        }
    });
